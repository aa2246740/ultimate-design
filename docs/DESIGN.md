...
description: Bilingual showcase homepage that proves the skill's design capability with the page itself - live-rendered specimens, a searchable knowledge explorer, and honest repository-sourced numbers.
...
This is the bilingual official showcase homepage for Ultimate Design, served from `docs/` on GitHub Pages. Version 0.8 replaces the quiet editorial page (0.7) with a capability showcase: the page must demonstrate design range, not only describe it. The proof devices are live-rendered style specimens, a searchable OKF knowledge explorer, an interactive five-step run record, and a meta strip that points back to this contract. Chinese is the default language; English is a parallel expression.
...
- `--paper` `#F2F3EE`: artifact sheets (run record, contract snippets, light specimens). Ink on paper: `#10120E`.
...
- Range gallery: six live-rendered specimens in distinct design languages, each labelled with its driving OKF concepts and taste dials.
...
- Major user-visible zones keep sparse `data-ud-check` markers: `navigation`, `hero-title`, `hero-lead`, `hero-run-record`, `stat-strip`, `observations`, `artifact-trio`, `workflow`, `okf-explorer`, `range-gallery`, `modes`, `install-tabs`, `prompt-console`, `delivery`, `meta-strip`, `footer`.
...
- Latest user override: None for this revision.
- Deliverable: Redesigned static bilingual homepage - `docs/index.html`, updated `docs/CONTENT.md` and `docs/DESIGN.md`, validation results, and rendered screenshots.
...
- Success criteria: A reader can name the three outputs and five steps, copy an install command and a prompt, filter the 54-entry knowledge explorer, and read either language; the page passes contract and OKF validation plus rendered review at mobile and desktop widths.
...
- Message hierarchy: 1. What the skill builds and keeps (claim plus run record). 2. Proof inventory (stats). 3. Why unreviewed pages fail. 4. The three outputs. 5. The five steps. 6. The knowledge base itself. 7. Design range. 8. Modes and routes. 9. Install and prompts. 10. Delivery and self-bootstrap proof.
...
- Trust, risk, and help content: The meta strip names the page's own contract files; specimen captions state they are rendered in-page; stats cite countable inventory only.
...
- Constraints extracted: page numbers must be countable in the repository; specimens must be honest in-page renders; dark field must hold WCAG AA for text roles; single static HTML file with inline data; Google Fonts is the only external dependency; motion must be transform/opacity, 400ms or less, and fully removable.
...
| `design-okf/systems/taste-engine.md` | Studio-desk-at-night direction: ink field, paper artifacts, red marks; locks against gradient-glow SaaS defaults and competitor signature | Global theme tokens, hero composition, specimen frames | Screenshots show a distinct identity; anti-default locks and the no-clone rule hold |
| `design-okf/systems/type-personality.md` | Fraunces plus Noto Serif SC display pairing with two weights each and full system fallback | Font loading, headings, specimen headlines | Page renders acceptably with fonts blocked; FOUT controlled via `display=swap`; weights limited |
...
| `design-okf/foundations/necessary-design-judgment.md` | Six specimens maximum, each captioned with its driving concepts; any element that demonstrates nothing is deleted | Range gallery, hero desk texture, ornament budget | Critique pass names what was cut and confirms every remaining element demonstrates a capability |
...
- Layout families: Editorial display hero with instrument panel, paper artifact cards, stepper rail, filterable card grid, framed specimen plates, tabbed console.
- Visual memory feature: Light paper sheets with red annotation marks on a deep ink desk, and six styled specimen plates proving range.
...
- Primary content: Claim, run record, stats, observations, three outputs, five steps, 54-entry explorer, six specimens, modes and routes, install and prompts, delivery checklist, meta strip.
...
- Visual: Hero dominates once; the explorer is the only dense zone; semantic zones do not overlap, clip, or overflow horizontally; specimens read as deliberate, distinct design languages.
...
- Performance: One HTML file around 250KB or less including inline data; Google Fonts with `display=swap` is the only external request; no other scripts, no raster images.
...
- CSS architecture: One static `index.html` with CSS custom properties mirroring the front matter, semantic class names, and scoped specimen styles that cannot leak (`.spec-*` prefixes per plate).
...
- Dark mode: The page is the dark mode; specimens may be locally light (paper plates).
...
- The orchestrator (Integrator) remains the sole writer of this contract; worker agents implement `index.html` and the data extract against it.

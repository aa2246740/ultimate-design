...
| Project-wide motion audit: surface inventory, runtime baseline, evidence vetting, leverage ranking, bounded repair plans, and post-fix replay | `branch-motion-audit.md`, `systems/motion-language.md`, `systems/motion-contract.md`, `quality-gates.md`, `scripts/inventory_motion.py` | Covered as an on-demand execution branch; static signals never become findings without contextual or runtime confirmation |
